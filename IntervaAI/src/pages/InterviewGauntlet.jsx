  ...
  const rounds = [
    {
      ...
      description: "This round benchmarks your mental agility. Expect a mix of number crunching, pattern recognition, and logical fallacies."
    },
     {
      id: 1,
      ...
      description: "Dive deep into the world of Data Structures. You'll be tested on your ability to optimize, refactor, and solve complex edge cases under pressure."
    },
    {
      id: 2,
      ...
      description: "The final frontier. We want to know the person behind the code. Expect questions about conflict resolution, career goals, and cultural alignment."
    }
  ];
        ...
        <div className="relative flex justify-between items-center px-4 md:px-20">
          ...
          {rounds.map((round, idx) => (
            <div key={idx} className="relative z-10 flex flex-col items-center group">
              ...
              >
                {idx < activeRound ? <CheckCircle2 className="text-emerald-400" /> : <span className="font-bold">{idx + 1}</span>}
              </button>
              <span className={`absolute -bottom-8 whitespace-nowrap text-xs font-bold uppercase tracking-tighter transition-colors ${idx <= activeRound ? 'text-indigo-400' : 'text-slate-500'}`}>
                {round.title.split(' ')[0]}
              </span>
            </div>
          ))}
      ...
      <main className="max-w-6xl mx-auto grid lg:grid-cols-12 gap-10 items-start">
        ...
        <div className="lg:col-span-8 space-y-8" key={activeRound}>
          <div data-aos="fade-right">
            <h4 className="text-indigo-400 font-black uppercase tracking-[0.3em] mb-2">Phase 0{activeRound + 1}</h4>
          ...
          <div className="grid sm:grid-cols-2 gap-4" data-aos="zoom-in-up" data-aos-delay="200">
            <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-3xl backdrop-blur-xl group hover:border-indigo-500/50 transition-all">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <p className="text-xs text-slate-500 uppercase font-bold tracking-widest">Time Limit</p>
                  <p className="text-2xl font-black text-white">{rounds[activeRound].duration}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-3xl backdrop-blur-xl group hover:border-emerald-500/50 transition-all">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <p className="text-xs text-slate-500 uppercase font-bold tracking-widest">Total Weightage</p>
                  <p className="text-2xl font-black text-white">{rounds[activeRound].marks} Points</p>
                </div>
              </div>
            </div>
          </div>

          <div className="bg-white/[0.02] border border-white/10 rounded-[2.5rem] p-8 md:p-10" data-aos="fade-up" data-aos-delay="400">
        ...
        <div className="lg:col-span-4 sticky top-12 space-y-6">
          ...
          >
            ...
            <div className="relative z-10 text-white transform group-hover:scale-125 transition-transform duration-700">
              {React.cloneElement(rounds[activeRound].icon, { size: 120, strokeWidth: 1 })}
            </div>
          </div>
          ...
          <div className="flex items-center justify-center gap-2 text-slate-500 text-xs font-bold uppercase tracking-widest" data-aos="fade-in">
